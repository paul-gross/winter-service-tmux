...
**"Here's how I'll wire your per-env services — for each: pane target, `cmd`, `cwd` (if any), port (if any), health probe (if any), and startup policy (if any); plus `env_file` if needed and the layout the hook will create. Confirm, or tell me what to change?"**
