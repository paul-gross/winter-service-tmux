...
- **A `[[service]]` may declare a `port` field** — either a literal integer (`port = 8080`) or an env-relative offset expression (`port = "WINTER_PORT_BASE + 10"`). A bare integer is an absolute port number (the same in every environment); the `WINTER_PORT_BASE + <offset>` form is resolved per-env at status time. Declared ports are surfaced in the `ports` field of winter's status document and the `PORTS` column of the rendered table — see `workspace:/ai/winter-cli/usage/service.md` for the wire shape. Services without `port` emit an empty `ports` field.
