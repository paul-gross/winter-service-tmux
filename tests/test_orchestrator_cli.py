...
# ---------------------------------------------------------------------------
# logs -f (follow) with more than one selected service → non-zero + message + no logs call
# ---------------------------------------------------------------------------


def test_main_logs_follow_multi_env_returns_nonzero(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """logs -f with a pattern spanning multiple envs (>1 service) → non-zero, message, no logs call."""
    fake = _install(
        monkeypatch,
        _FakeContainer(sessions=["mp-alpha", "mp-beta"]),
    )
    ...
    # "*/backend" matches backend in both alpha and beta → two services → follow guard fires
    rc = main(["logs", "*/backend"])

    assert rc != 0
    err = capsys.readouterr().err
    assert "follow" in err.lower()
    assert len(fake.logs_calls) == 0


def test_main_logs_follow_multi_service_single_env_returns_nonzero(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """logs -f selecting >1 service within ONE env → non-zero, message, no logs call."""
    fake = _install(
        monkeypatch,
    ...
    # "alpha/*" matches both backend and worker in alpha → two services → follow guard fires
    rc = main(["logs", "alpha/*"])
...
def test_main_logs_follow_single_service_calls_logs(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """logs -f selecting exactly one service → the follow guard does not fire; logs is called once."""
    fake = _install(
        monkeypatch,
        _FakeContainer(sessions=["mp-alpha"]),
    )
