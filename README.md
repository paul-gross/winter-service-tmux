...
- **Declarative TOML manifest** — a single `workspace:/.winter/config/winter-service-tmux/config.toml` declares the panes, commands, and session prefix; the orchestrator is generic and the project owns the layout. An optional gitignored `config.local.toml` in the same directory overlays per-machine overrides on top.
