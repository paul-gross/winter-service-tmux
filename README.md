...
- **Generated agent map** — the agent-facing `setup-tmux.md` (service name → `<window>.<pane>` target) is rendered from `setup-tmux.sh` by `render-setup-md.sh`, so the map agents read can't drift from the config. No hand-maintained second source of truth; `winter doctor` flags it if the two fall out of sync.
