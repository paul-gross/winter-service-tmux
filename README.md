...
- **Declarative manifest groundwork** — an optional `workspace:/ai/project/setup-tmux.toml` can describe the same services as `setup-tmux.sh` in a structured, machine-readable format, alongside an optional gitignored `setup-tmux.local.toml` overlay. The manifest and a stdlib-only Python reader/validator are additive groundwork for a future Python orchestrator (issue #6); the bash `setup-tmux.sh` remains the live config until that cutover. `winter doctor` Probe 5 validates the TOML manifest when present. See `workflow/setup-tmux.toml.example` for the annotated schema.
