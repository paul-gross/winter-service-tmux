...
| `on_env_destroy` | `hooks/destroy-worktree.sh` | Before `winter ws destroy <env>` removes any per-repo worktree or the env dir | Sources `workspace:/ai/project/workflow.sh` to derive `SESSION_PREFIX`, then runs the extension's own `./down` for the env (falls back to `tmux kill-session` if `./down` is unavailable). Idempotent: exits 0 if the session is already gone. |
