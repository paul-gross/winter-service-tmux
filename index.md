...
- **To recover a single wedged or crashed service, use `./restart <service>`** — the sanctioned alternative to a manual `kill`/`pkill` or a full `./down && ./up`. It reaps just that service's pane and re-runs its declared command, leaving every other pane in the session running. The argument is a *declared service name*, not an env or worktree; scope comes from *which* env's `./restart` you invoke, the same as `./up`/`./down`/`./status`.
