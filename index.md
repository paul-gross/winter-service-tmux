...
Files in this extension are addressed with the `winter-service-tmux:` prefix — for example, `winter-service-tmux:/ai/service-rules.md`. Resolve to the on-disk path via the `# Winter Extensions` block in workspace `CLAUDE.md`.
...
| Topic | Read when… |
|-------|------------|
| [Service management rules](./ai/service-rules.md) | …you need the operating rules: `./up`/`./down`/`./restart`/`./status` conventions, log commands and examples, log modes (`file`/`pane`/`memory`) |
| [Workspace-scoped singleton services](./ai/workspace-singletons.md) | …you need to drive or declare shared workspace services (`winter service … workspace`, `scope = "workspace"`) |
| [Log capture configuration](./ai/log-capture.md) | …you need to tune log rotation, understand mixed-mode output, or configure the `[logs]` table |
| [Orchestrator dev loop](./ai/orchestrator-dev-loop.md) | …you are developing the orchestrator and need to test in-progress code against a live worktree |
| [Workflow setup](./ai/workflow-setup.md) | …you are setting up a new workspace and need to author `config.toml` and `layout-hook.sh` |
